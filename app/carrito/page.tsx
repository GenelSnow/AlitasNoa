...
export default function CarritoPage() {
    ...
    function buildWhatsAppMessage(codigoUsado?: string | null) {
        ...
        return [
            "¡Hola AlitasNOA! Quiero hacer este pedido:",
            "",
            ...lineas,
            "",
    ...
    async function handleEnviar(e: React.FormEvent) {
        ...
        try {
            ...
            if (codigoReferido.trim()) {
                ...
                // 1) Sin sesión → no digas "no existe"
                if (!user) {
                ...
                // 2) Buscar el código (con RLS corregido o vía RPC)
                const { data: rows, error: refError } = await supabase
                    .rpc("buscar_referidor", { p_codigo: codigo })
            ...
            if (user) {
                const payload = {
                    cliente_id: user.id,
                    subtotal: total,                    // solo productos
                    costo_domicilio: precioDomicilio,   // 2500 (o lo de config_app)
                    total: totalConDomicilio,           // productos + domicilio
                    forma_pago: formaPago,              // efectivo | nequi | llave
            ...
            // Mensaje WhatsApp (incluye código si hubo)
            const text = encodeURIComponent(
            buildWhatsAppMessage(codigoUsado)
        )
